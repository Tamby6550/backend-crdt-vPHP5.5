...
class Facture extends Controller
{
    ...
    {     
        ...
        (SELECT distinct bill.MONTANT_PATIENT   FROM MIANDRALITINA.BILLING1 bill WHERE NUM_FACT=RRF.NUM_FACT  AND bill.DATE_ARRIV=RRF.DATE_ARRIV AND bill.NUM_ARRIV=RRF.NUM_ARRIV ) as totalpat,
        (SELECT distinct bill.MONTANT_PEC   FROM MIANDRALITINA.BILLING1 bill WHERE NUM_FACT=RRF.NUM_FACT AND bill.DATE_ARRIV=RRF.DATE_ARRIV AND bill.NUM_ARRIV=RRF.NUM_ARRIV ) as totalpec,
        (SELECT distinct bill.MONTANT_PATIENT_REGLE   FROM MIANDRALITINA.BILLING1 bill WHERE NUM_FACT=RRF.NUM_FACT AND bill.DATE_ARRIV=RRF.DATE_ARRIV AND bill.NUM_ARRIV=RRF.NUM_ARRIV ) as rpatient,
        (SELECT distinct bill.MONTANT_PEC_REGLE   FROM MIANDRALITINA.BILLING1 bill WHERE NUM_FACT=RRF.NUM_FACT AND bill.DATE_ARRIV=RRF.DATE_ARRIV AND bill.NUM_ARRIV=RRF.NUM_ARRIV ) as rclient
        ...
        AND trunc(RRF.DATE_FACTURE)>=trunc(sysdate) 
    ...
    {    
        ...
        if ($nom_client!="" )          {
            $sql=$sql." AND (SELECT distinct bill.CLIENT   FROM MIANDRALITINA.BILLING1 bill WHERE NUM_FACT=RRF.NUM_FACT  AND upper(bill.CLIENT) 
            like upper('%".$nom_client."%') ) like upper('%".$nom_client."%')";
        }

        //Recherche izay misy prise en charge
        if ($pec)   {
            $sql=$sql." AND (SELECT distinct bill.PEC   FROM MIANDRALITINA.BILLING1 bill WHERE NUM_FACT=RRF.NUM_FACT  AND bill.PEC>0) >0";
        }
